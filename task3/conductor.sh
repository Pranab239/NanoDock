...
run()
{
    ...
    chmod 755 $CONTAINERDIR/$NAME/rootfs    # setting permission to the root dir
    # mount -t proc /proc $CONTAINERDIR/$NAME/rootfs/proc
    mount -t sysfs /sys $CONTAINERDIR/$NAME/rootfs/sys

    # mount the sbin for running the ifconfig command
    # sudo mount --bind /sbin $CONTAINERDIR/$NAME/rootfs/sbin

    unshare --fork --uts --pid --net --mount --ipc --kill-child --mount-proc=$CONTAINERDIR/$NAME/rootfs/proc \
...
addnetwork()
{
    ...
    # Enable interfaces within the host
    ip link set $OUTSIDE_PEER up

    # Enable interfaces within the container
    nsenter --target $PID --net ip link set $INSIDE_PEER up

    # Enable loopback interface within the container
